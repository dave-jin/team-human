...
- **The press numbers do not match ours, on purpose.** Post-event articles say `44 teams / 약 100명` — that is the pre-event press release's figure, written before the arena closed. Our result report is the later and stricter count. **Do not "fix" 41 to 44.** This is also why the Platum piece (`5시간 만에 움직이는 시제품 44개…`) is **deliberately not in the press list** despite being the best post-event write-up: its headline number would contradict our own recap one scroll away.
...
- **Verify before adding.** Every URL was fetched in a real browser and confirmed to return 200 over strict TLS *and* to actually name Team Human (or 커서 코리아 커뮤니티, which is what vol.1 coverage called the community before the Team Human brand existed). Outlets that did cover events but stay dropped: **워크투데이** (cert is for `headlinejeju.co.kr`), **smartn.tech** (expired cert), **beSUCCESS** (permanent Cloudflare challenge — unverifiable). A link that throws a browser warning is worse than no link. Three more were checked for Toython and left out for content reasons, not TLS: **AI타임스** ran the same release but omits Team Human entirely, **테크월드** loads two `http://` sub-resources inside an HTTPS page, and **플래텀** carries the 44-team figure (see the Recap section).
...
- **Hero slider** — the `.hero-slider` inside `.hero` is a scroll-snap carousel with 3 slides (vol.3 group photo + earlier group photo + YouTube recap), navigated by dots, prev/next arrows, swipe, and keyboard left/right. Slide 1 is `images/hero-toython-group.jpg` (2026-09-06; it replaced `hero-vol3-group.jpg`, which stays on disk). It is the Toython group shot under the TOY-THON LED wall, resized to the slider's `16:9` and **anchored to the bottom** — the ceiling is empty and the people are not. **Slide 1 should be the most recent event**; that is the whole reason it changes. Only slide 1 loads eagerly; the rest carry `loading="lazy"`. Slider logic lives in `i18n.js → initHeroSlider()`. To rotate the recap video, change the iframe `src` in `index.html` and update `metadata.videoId` + `metadata.url` on `Hero Slider → Slide 2` in `.pen`. To swap the hero photo, replace `images/hackathon-hero.png` (or update the `src` in both `index.html` and the slider's `Slide 1` frame). When adding/removing slides, keep the dot count (`.hero-slider-dot`) and the .pen `Slider Dots.metadata.items` in sync.
